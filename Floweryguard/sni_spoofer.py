"""Модуль генерации фейковых TLS ClientHello и подмены SNI (экспериментальный).

ВАЖНОЕ АРХИТЕКТУРНОЕ ПРИМЕЧАНИЕ:
На уровне пользовательского TCP-прокси (socket.SOCK_STREAM) без драйвера перехвата пакетов 
(WinDivert / NFQUEUE) отправка фейкового ClientHello в реальное TCP-соединение приводит к 
разрыву сессии сервером из-за нарушения TLS RFC (два ClientHello подряд).
Поэтому в Flowery v1.2.0 данные техники по умолчанию отключены в config.ini (fake_sni_record = False, 
spoof_sni = False). Основной и полностью рабочий обход ТСПУ осуществляется через проверенные техники:
TCP Split, TLS Record Fragmentation и OOB Data (TCP Urgent).
"""
